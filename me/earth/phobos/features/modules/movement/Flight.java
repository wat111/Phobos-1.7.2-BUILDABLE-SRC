    ...
    public void onUpdateWalkingPlayer(UpdateWalkingPlayerEvent event) {
        ...
        if (event.getStage() == 0) {
            ...
            if (this.mode.getValue() == Mode.VANILLA) {
                ...
                if (Flight.mc.gameSettings.keyBindJump.isKeyDown()) {
                    Flight.mc.player.motionY = this.noKick.getValue().booleanValue() ? (Flight.mc.player.ticksExisted % 20 == 0 ? (double)-0.04f : (double)this.speed.getValue().floatValue()) : (Flight.mc.player.motionY += (double)this.speed.getValue().floatValue());
                }
                if (Flight.mc.gameSettings.keyBindSneak.isKeyDown()) {
